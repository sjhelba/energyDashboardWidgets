function defineFuncForTabSpacing() {
  ...
  const setupDefinitions = () => {
    let cx, cy, width, height, startAngle, endAngle, gaugeArcOuterRadius,
      ...
      angleScale, gaugeArcGenerator, backgroundArcGenerator,
    ...
    [minVal, maxVal] = [data.minVal, data.maxVal];
  ...
  const renderWidget = data => {
    let {
      ...
      gaugeArcThickness, titleFont, unitsFont, valueFont, backgroundColor, gaugeArcColor, subTargetGaugeArcColor,
    ...
    const gaugeArc = chartGroup.append('path')
      ...
      // Starts with min val color prior to transition
      .attr('fill', gaugeArcColor)
