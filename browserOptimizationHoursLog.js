...
const overallOuterRadius = 100;
const overallInnerRadius = 60;
const overallArcThickness = overallOuterRadius - overallInnerRadius;
...
const allDonutGroupsGroup = graphicGroup.append('g')
    ...
    .attr('transform', `translate(${margin.left + (moduleOuterRadius * 2)}, ${margin.top + legendHeight + paddingUnderLegend + moduleOuterRadius})`)
...
const overallArcPaths = overallDonutGroup.selectAll('.overallPath')
    ...
    .enter().append('path')
        ...
        .style('fill-opacity', (d, i) => active[overallData[i].category] ? 1 : 0.5);
...
const optimizedPaths = d3.selectAll('.optimizedPath')
    ...
    .on('mouseleave', function(){
        ...
        optimizedPaths.style('fill-opacity', 0.5);
        renderTooltip()
    });
const standardPaths = d3.selectAll('.standardPath')
    ...
    .on('mouseleave', function(){
        ...
        standardPaths.style('fill-opacity', 0.5);
        renderTooltip()
    });
