function defineFuncForTabSpacing () {
	...
	const renderWidget = data => {
		...
		dropdownDiv.append('select')		// assume 5px above and 5 px below textHeight for dropdownHeight
			...
			.style('padding', '5px')
			.style('background-color', widget.yearDropdownHovered ? 'white' : data.dropdownFillColor)
...
// make box of background color with slight opacity to blur background and then add modal on top
		...
		const settingsButton = chartGroup.append('svg:image')
			...
			.attr('x', data.graphicWidth - data.settingsBtnSize)
			.attr('y', -(data.settingsBtnSize + data.paddingBelowDropdown))
			.attr('height', data.settingsBtnSize / 2)
		...
		function renderModal () {
			...
			modalGroup.append('rect')
				...
				.attr('rx', data.tooltipRoundedness * 2)
				.attr('ry', data.tooltipRoundedness * 2)
